...
export function isRolesHigher(roles: Role[], target: Role[]): boolean {
	return roles.some((role) => target.every((targetRole) => isRoleHigher(role, targetRole)));
}
...
export function isRolesAtLeast(roles: Role[], target: Role[]): boolean {
	return roles.some((role) => target.every((targetRole) => isRoleAtLeast(role, targetRole)));
}
